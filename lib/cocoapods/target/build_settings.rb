    ...
    class BuildSettings
      ...
      class PodTargetSettings < BuildSettings
        ...
        define_build_settings_method :dynamic_frameworks_to_import, :memoized => true do
          ...
          # TODO: Check for static linkage in the xcframework slices
          dynamic_frameworks_to_import = vendored_xcframeworks.map { |f| File.basename(f, '.xcframework') }
        ...
        define_build_settings_method :vendored_framework_search_paths, :memoized => true do
          ...
          xcframeworks = file_accessors
                                .flat_map(&:vendored_xcframeworks)
                                .flat_map { |path| Xcode::XCFramework.new(path) }
          search_paths.concat xcframeworks
                                .flat_map(&:slices)
                                .reject { |slice| slice.platform.symbolic_name != target.platform.symbolic_name }
